...
| 特徴 | 詳細 | 技術的価値 |
|------|------|-----------|
| **🤖 個人化レシピシステム** | システムプロンプトで「いつものレシピ」を学習<br/>「いつも」キーワードで個人の定番料理を優先提案 | SLM活用による個人適応型AI |
...
| **⚡ ストリーミング応答** | リアルタイム文字表示でスムーズな対話体験<br/>Server-Sent Eventsによる高速レスポンス | 高品質ユーザー体験の実現 |

### 📊 技術仕様

| 項目 | 仕様 | 特徴 |
|------|------|------|
| **AI エンジン** | ローカルSLM + LM Studio | 使用制限なし・完全プライベート |
...
- **料理特化プロンプト** - 豊富なレシピデータベース内蔵（GitHub版は代表3つのみ掲載）
- **OpenAI互換API** - /v1/chat/completions エンドポイントでストリーミング応答

> 📝 **レシピデータについて**: 実際の運用では多数のレシピを内蔵していますが、GitHubの`システムプロンプトの例.md`では代表的な3つのレシピ（しょうが焼き・なす照り焼き・ハンバーグ）のみを公開しています。
